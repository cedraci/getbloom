...
pub async fn detect_view_gaps(state: State<'_, AppState>, view_id: i64)
    ...
    Ok(scheduler::detect_gaps(&state.pool, view_id, 30, today).await?
        .into_iter()
        // P11 11.5, Task 7: period-shaped gaps are now reported alongside the
        // daily ones. A period's bounds are not a day range -- a 31-day month
        // trips BACKFILL_CAP_DAYS, and a shorter one would buy a month of
        // DAILY history that can never close the gap (periodic fields are
        // excluded from the daily leg by design). The UI renders `period`'s
        // label instead of `start`/`end` and disables its Backfill button:
        // the scheduler re-buys due periods automatically (`gap_backfill`),
        // and a daily backfill can never close a period-shaped hole.
